...
function officeForCitizen(person, ministers) {
  const given = normalizePersonName(person.given_name);
  const family = normalizePersonName(person.family_name);
  const full = `${given} ${family}`.trim();
  const match = (ministers || []).find((minister) => {
    const name = normalizePersonName(minister.name);
    if (!name) return false;
    if (full === name || given === name) return true;
    if (name.includes(" ")) {
      return full.startsWith(`${name} `) || given === name;
    }
    return given.split(" ")[0] === name;
  });
  return match?.title || "";
}

async function citizensWithOffices(sql, rows, mapRow) {
  const ministers = await sql`select name, title from ministers order by sort_order`;
  return rows.map((row) => ({
    ...mapRow(row),
    office: officeForCitizen(row, ministers),
  }));
}
  ...
  try {
    ...
    const rows = await sql`
      select id, given_name, family_name, dob, region, email, address, role, status,
             review_note, reviewed_at, current_citizenship, dual_citizenship_reason,
             leadership_qualities, join_reason, knows_official, official_name,
             recommendation, applied_at, created_at, password_hash, blacklisted, blacklist_note, blacklisted_at
    ...
    if (account.blacklisted) {
      return res.status(403).json({
        ok: false,
        error: "This account is barred from Cheria services.",
      });
    }
  ...
  try {
    ...
    const rows = await sql`
      select id, given_name, family_name, dob, region, email, address, role, status,
             review_note, reviewed_at, current_citizenship, dual_citizenship_reason,
             leadership_qualities, join_reason, knows_official, official_name,
             recommendation, applied_at, created_at, blacklisted, blacklist_note, blacklisted_at
...
export async function handleMe(req, res) {
  ...
  if (!account) return;
  const token = issueAuth(res, account);
  return res.status(200).json({ ok: true, token, citizen: publicAccount(account) });
}
...
export async function handleAdminPage(req, res) {
  ...
  try {
    const sql = await getReadySql();
    const rows = await sql`
      select id, role from citizens where id = ${session.id}
    `;
    const account = rows[0];
    if (!account) {
      clearAuthCookie(res);
    ...
    const templatePath = path.join(path.dirname(fileURLToPath(import.meta.url)), "..", "templates", "admin.html");
...
export async function handleApplication(req, res) {
  ...
  if (account.blacklisted) {
    return res.status(403).json({ ok: false, error: "This account is barred from Cheria services." });
  }
  ...
  try {
    ...
    const rows = await sql`
      select id, given_name, family_name, dob, region, email, address, role, status,
             ...
             recommendation, applied_at, created_at
      from citizens
      where coalesce(role, 'citizen') <> 'admin'
        and coalesce(blacklisted, false) = false
        and (
          status in ('pending', 'approved', 'rejected')
          or applied_at is not null
          or coalesce(join_reason, '') <> ''
        )
  ...
  try {
    ...
    const updated = await sql`
      ...
      set status = ${status},
          review_note = ${note},
          reviewed_at = now()
      where id = ${id} and coalesce(role, 'citizen') <> 'admin'
      returning id, given_name, family_name, dob, region, email, address, role, status,
                review_note, reviewed_at, current_citizenship, dual_citizenship_reason,
                leadership_qualities, join_reason, knows_official, official_name,
                recommendation, applied_at, created_at
...
export async function handleAdminBlacklistGet(req, res) {
  const admin = await requireAdmin(req, res);
  if (!admin) return;
  try {
    const sql = await getReadySql();
    const blacklisted = await sql`
      select id, given_name, family_name, dob, region, email, address, role, status,
             ...
             recommendation, applied_at, created_at, blacklisted, blacklist_note, blacklisted_at
      from citizens
      where coalesce(role, 'citizen') <> 'admin' and coalesce(blacklisted, false) = true
      order by blacklisted_at desc, created_at desc
    `;
    const registered = await sql`
      select id, given_name, family_name, dob, region, email, address, role, status,
             ...
             recommendation, applied_at, created_at, blacklisted, blacklist_note, blacklisted_at
      from citizens
      where coalesce(role, 'citizen') <> 'admin' and coalesce(blacklisted, false) = false
      order by family_name, given_name
    `;
    return res.status(200).json({
      ok: true,
      blacklisted: blacklisted.map(publicAccount),
      registered: registered.map(publicAccount),
    });
  } catch (err) {
    console.error("admin blacklist list failed", err);
    return res.status(500).json({ ok: false, error: "Could not load the blacklist." });
  }
}

export async function handleAdminBlacklistUpdate(req, res) {
  const admin = await requireAdmin(req, res);
  if (!admin) return;
  const body = req.body || {};
  const action = String(body.action || "").trim().toLowerCase();
  ...
  if (!id && !email) {
    return res.status(400).json({ ok: false, error: "Select a registered individual or enter their email." });
  }
  try {
    const sql = await getReadySql();
    const rows = id
      ? await sql`select id, role, email from citizens where id = ${id}`
      : await sql`select id, role, email from citizens where email = ${email}`;
    const person = rows[0];
    if (!person) {
      return res.status(404).json({ ok: false, error: "No registered individual matches that record." });
    }
    if (person.role === "admin") {
      return res.status(403).json({ ok: false, error: "Ministry administrators cannot be blacklisted." });
    }
    const blacklisted = action === "add";
    const updated = blacklisted
      ? await sql`
          update citizens
          set blacklisted = true,
              blacklist_note = ${note},
              blacklisted_at = now()
          where id = ${person.id} and coalesce(role, 'citizen') <> 'admin'
          returning id, given_name, family_name, dob, region, email, address, role, status,
                    review_note, reviewed_at, current_citizenship, dual_citizenship_reason,
                    leadership_qualities, join_reason, knows_official, official_name,
                    recommendation, applied_at, created_at, blacklisted, blacklist_note, blacklisted_at
    ...
    console.error("admin blacklist update failed", err);
    return res.status(500).json({ ok: false, error: "Could not update the blacklist." });
  }
}

export async function handlePublicCitizens(_req, res) {
  try {
    const sql = await getReadySql();
    const rows = await sql`
      select given_name, family_name, region, created_at
      from citizens
    ...
    return res.status(200).json({
      ok: true,
      citizens: await citizensWithOffices(sql, rows, (row) => ({
    ...
    console.error("public citizens failed", err);
    return res.status(500).json({ ok: false, error: "Could not load the citizen roll." });
  }
}

export async function handleAdminCitizens(req, res) {
  const admin = await requireAdmin(req, res);
  if (!admin) return;
  try {
    const sql = await getReadySql();
    ...
    return res.status(200).json({
      ok: true,
      citizens: await citizensWithOffices(sql, rows, publicAccount),
    });
  } catch (err) {
    console.error("admin citizens failed", err);
    return res.status(500).json({ ok: false, error: "Could not load the citizen roll." });
  }
}
